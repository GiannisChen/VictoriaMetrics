...
func TestGreetingUnPack(t *testing.T) {
	...
	// normal
	{
		...
		assert.Equal(t, SERVER_STATUS_AUTOCOMMIT, int(got.Status()))
	}

	// 1. off CLIENT_PLUGIN_AUTH
	{
		want.Capability = want.Capability &^ CLIENT_PLUGIN_AUTH
		want.authPluginName = "mysql_native_password"
		err := got.UnPack(want.Pack())
		assert.Nil(t, err)
		assert.Equal(t, want, got)
	...
	// 2. off CLIENT_SECURE_CONNECTION
	{
		want.Capability &= ^CLIENT_SECURE_CONNECTION
		want.authPluginName = "mysql_native_password"
		err := got.UnPack(want.Pack())
		assert.Nil(t, err)
		assert.Equal(t, want, got)
	...
	// 3. off CLIENT_PLUGIN_AUTH && CLIENT_SECURE_CONNECTION
	{
		want.Capability &= ^CLIENT_PLUGIN_AUTH ^ CLIENT_SECURE_CONNECTION
		want.authPluginName = "mysql_native_password"
		err := got.UnPack(want.Pack())
		assert.Nil(t, err)
		assert.Equal(t, want, got)
