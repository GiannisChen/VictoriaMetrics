...
var (
	...
	rowsTenantInserted = tenantmetrics.NewCounterMap(`vm_tenant_inserted_rows_total{type="vmsql"}`)
	rowsPerInsert      = metrics.NewHistogram(`vm_rows_per_insert{type="vmsql"}`)
)

// insertHandler processes /api/v1/sql INSERT part.
func insertHandler(at *auth.Token, stmt *vmsql.InsertStatement) error {
...
func ParseStmt(stmt *vmsql.InsertStatement, table *vmsql.Table, callback func(rows []Row) error) error {
...
func ParseColumnDescriptors(stmt *vmsql.InsertStatement, table *vmsql.Table) ([]ColumnDescriptor, error) {
...
func insertRows(at *auth.Token, rows []Row) error {
