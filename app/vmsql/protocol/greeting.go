...
type Greeting struct {
	...
	// SERVsqlerror.ER_STATUS_AUTOCOMMIT is default.
	status uint16
...
func (g *Greeting) UnPack(payload []byte) error {
	...
	if g.protocolVersion, err = buf.ReadU8(); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting protocol-version failed")
	}

	// string[NUL]: server version
	if g.serverVersion, err = buf.ReadStringNUL(); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting server-version failed")
	}

	// 4: connection id
	if g.ConnectionID, err = buf.ReadU32(); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting onnection-id failed")
	}

	// string[8]: auth-plugin-data-part-1
	var salt8 []byte
	if salt8, err = buf.ReadBytes(8); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting auth-plugin-data-part-1 failed")
	}
	copy(g.Salt, salt8)

	// 1: [00] filler
	if err = buf.ReadZero(1); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting filler failed")
	}

	// 2: capability flags (lower 2 bytes)
	var capLower uint16
	if capLower, err = buf.ReadU16(); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting capability-flags failed")
	}

	// 1: character set
	if g.Charset, err = buf.ReadU8(); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting charset failed")
	}

	// 2: status flags
	if g.status, err = buf.ReadU16(); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting status-flags failed")
	}

	// 2: capability flags (upper 2 bytes)
	var capUpper uint16
	if capUpper, err = buf.ReadU16(); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting capability-flags-upper failed")
	}
	g.Capability = (uint32(capUpper) << 16) | (uint32(capLower))

	// 1: length of auth-plugin-data-part-1
	var SLEN byte
	if (g.Capability & CLIENT_PLUGIN_AUTH) > 0 {
		if SLEN, err = buf.ReadU8(); err != nil {
			return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting auth-plugin-data length failed")
		}
	} else {
		if err = buf.ReadZero(1); err != nil {
			return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting zero failed")
		}
	}

	// string[10]: reserved (all [00])
	if err = buf.ReadZero(10); err != nil {
		return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting reserved failed")
	}

	// string[$len]: auth-plugin-data-part-2 ($len=MAX(13, length of auth-plugin-data - 8))
	if (g.Capability & CLIENT_SECURE_CONNECTION) > 0 {
		read := int(SLEN) - 8
		if read < 0 || read > 13 {
			read = 13
		}
		...
		if salt2, err = buf.ReadBytes(read); err != nil {
			return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting salt2 failed")
		}

		// The last byte has to be 0, and is not part of the data.
		if salt2[read-1] != 0 {
			return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting auth-plugin-data-part-2 is not 0 terminated")
		}
	...
	if (g.Capability & CLIENT_PLUGIN_AUTH) > 0 {
		if g.authPluginName, err = buf.ReadStringNUL(); err != nil {
			return sqlerror.NewSQLError(sqlerror.ER_MALFORMED_PACKET, "extracting greeting auth-plugin-name failed")
		}
	}
	return nil
}
