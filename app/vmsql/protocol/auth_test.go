...
func TestAuthUnPack(t *testing.T) {
	...
	want.clientFlags |= CLIENT_CONNECT_WITH_DB
...
func TestAuthWithoutSecure(t *testing.T) {
	...
	want.charset = 0x02
	want.authResponseLen = 20
	want.clientFlags = DefaultClientCapability &^ CLIENT_SECURE_CONNECTION &^ CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA
	want.clientFlags |= CLIENT_CONNECT_WITH_DB
	...
	got := NewAuth()
	err := got.UnPack(want.Pack(
		DefaultClientCapability&^CLIENT_SECURE_CONNECTION,
		0x02,
		"root",
		"password",
		DefaultSalt,
...
func TestAuthWithoutPluginAuth(t *testing.T) {
	...
	want.charset = 0x02
	want.authResponseLen = 20
	want.clientFlags = DefaultClientCapability ^ CLIENT_PLUGIN_AUTH
	want.authResponse = nativePassword("sbtest", DefaultSalt)
	want.user = "sbtest"
	want.database = "test_db"
